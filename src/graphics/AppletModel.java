		...
		for (int i = 0; i < currentBranch.size(); i++) {
			...
			for (int j = 0; j < branch; j++) {
				...
				for (int k = 0; k < lineList.size() - 1; k++)
					tools.drawLine(lineList.get(k).multiple(magnif), lineList
			...
			if (branch < branchList.size()) {
				...
				tools.drawLine(lineList.get(line).multiple(magnif),
						finalPoint.multiple(magnif));
				applet.pushMatrix();
				applet.translate((float) finalPoint.x * magnif,
						(float) finalPoint.y * magnif, (float) finalPoint.z
								* magnif);
	...
	public void drawSurfaces() {
		...
		for (Surface s : surfaces) {
			if (s instanceof Ribbon) {
				Ribbon r = (Ribbon) s;
				float xMapping = 1, yMapping = (float) (xMapping * CNTimg.width
						* zBounds / (r.b * CNTimg.height));
				applet.beginShape();
				applet.texture(CNTimg);
				tools.vertex(new Vector(r.rx, r.ry, 0).multiple(magnif), 0,
						yMapping);
				tools.vertex(new Vector(r.x2, r.y2, 0).multiple(magnif),
						xMapping, yMapping);
				tools.vertex(new Vector(r.x2, r.y2, zBounds).multiple(magnif),
						xMapping, 0);
				tools.vertex(new Vector(r.rx, r.ry, zBounds).multiple(magnif),
						0, 0);
