...
public class Triangle extends Surface {
	...
	double c, rx2, ny0, nx1, ny1, nx2, ny2;

	public Triangle(Vector v1, Vector v2, Vector v3, Vector center) {
		p1 = v1.clone();
		p2 = v2.clone();
		p3 = v3.clone();
		norm = V.sub(v1, v2).cross(V.sub(v2, v3)).normalize();
		if (V.dot(norm, V.sub(v1, center)) < 0) norm = norm.multiple(-1);
		n1 = V.sub(v2, v1).normalize();
		n2 = norm.cross(n1).normalize();
		c = V.dot(p1, norm);
	...
	public double collisionDistance(Photon p) {
		...
		if (k>0 || d < 0) return Double.POSITIVE_INFINITY;
